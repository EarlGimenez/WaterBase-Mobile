  ...
  return (
    <SafeAreaView className="flex-1 bg-waterbase-50">
      ...
      <View className="flex-1 relative">
        {/* Top Controls Bar */}
        <View className="absolute top-4 left-4 right-4 z-10 flex-row items-center space-x-3">
          {/* Mode Toggle */}
          <View className="flex-row bg-white rounded-full shadow-lg overflow-hidden">
            <TouchableOpacity
              className={`px-4 py-2 ${viewMode === "reports" ? "bg-waterbase-500" : "bg-white"}`}
              onPress={() => setViewMode("reports")}
            >
              <Text className={`text-sm font-medium ${viewMode === "reports" ? "text-white" : "text-gray-700"}`}>
                Reports
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              className={`px-4 py-2 ${viewMode === "research" ? "bg-waterbase-500" : "bg-white"}`}
              onPress={() => setViewMode("research")}
            >
              <Text className={`text-sm font-medium ${viewMode === "research" ? "text-white" : "text-gray-700"}`}>
                Research
              </Text>
            </TouchableOpacity>
          </View>
          ...
          <View className="flex-1 relative">
            ...
            <TextInput
              ...
              className="bg-white rounded-full pl-10 pr-4 py-2 text-sm shadow-lg"
            />
          </View>
        ...
        >
          ...
          {viewMode === "reports" && showLayers.pollution && filteredReports.map((report) => (
            <Marker
              key={report.id}
              coordinate={{
            ...
            >
              <View
                ...
                style={{ backgroundColor: getSeverityColor(report.severityByUser) }}
              >
          ...
          {(viewMode === "research" || showLayers.sensors) && mockSensorStations.map((sensor) => (
            <Marker
              key={sensor.id}
              coordinate={{
            ...
            >
              <View
                ...
                style={{ backgroundColor: getWQIColor(sensor.waterQualityIndex) }}
              >
                <Text className="text-white text-xs font-bold">{sensor.waterQualityIndex}</Text>
              </View>
            </Marker>
          ))}
        </MapView>
        ...
        {/* Map Legend */}
        <View className="absolute top-20 left-4 z-10 bg-white rounded-lg shadow-lg p-3 max-w-32">
          <Text className="text-xs font-bold text-gray-800 mb-2">Legend</Text>
          ...
          {viewMode === "reports" ? (
            ...
            <View className="space-y-1">
              <View className="flex-row items-center">
                <View className="w-3 h-3 rounded-full bg-green-500 mr-2" />
                <Text className="text-xs text-gray-600">Good WQI</Text>
              </View>
              <View className="flex-row items-center">
                <View className="w-3 h-3 rounded-full bg-yellow-500 mr-2" />
                <Text className="text-xs text-gray-600">Fair WQI</Text>
              </View>
              <View className="flex-row items-center">
                <View className="w-3 h-3 rounded-full bg-orange-500 mr-2" />
                <Text className="text-xs text-gray-600">Poor WQI</Text>
              </View>
              <View className="flex-row items-center">
                <View className="w-3 h-3 rounded-full bg-red-500 mr-2" />
                <Text className="text-xs text-gray-600">Very Poor</Text>
              </View>
            </View>
          )}
          ...
          <View className="mt-3 pt-2 border-t border-gray-200">
            <Text className="text-xs font-bold text-gray-800 mb-2">Layers</Text>
            <View className="space-y-1">
              <TouchableOpacity 
                className="flex-row items-center justify-between"
                onPress={() => setShowLayers(prev => ({ ...prev, pollution: !prev.pollution }))}
              >
                <Text className="text-xs text-gray-600">Pollution</Text>
                <Switch
              ...
              <TouchableOpacity 
                className="flex-row items-center justify-between"
                onPress={() => setShowLayers(prev => ({ ...prev, sensors: !prev.sensors }))}
              >
                <Text className="text-xs text-gray-600">Sensors</Text>
                <Switch
