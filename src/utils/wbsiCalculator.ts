...
export class WBSICalculator {
    ...
    calculateThresholdAnalysis(severities: number[], weights: number[], consensus: number, nReports: number, polymodal: boolean, severityBands: any): any {
        ...
        const totalWeight = Object.values(severityBands).reduce((sum: number, val: any) => sum + (val as number), 0);
        const criticalRatio = totalWeight > 0 ? (severityBands.critical as number) / 100 : 0;
        const combinedSeriousRatio = totalWeight > 0 ? ((severityBands.critical as number) + (severityBands.high as number)) / 100 : 0;
